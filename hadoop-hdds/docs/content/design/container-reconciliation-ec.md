...
## Nomenclature
1. BlockGroup: A group of same blockId across data and parity nodes.
2. Stripe: A row of data and parity chunks within a BlockGroup.
3. ReplicaIndex: The ordered index of replica that comprise an EC container. For example, in a 3-2 EC layout, the replica indexes are 1, 2, 3 for data nodes and 4, 5 for parity nodes.
4. StripeChecksum: A checksum is generated by concatenating the list of checksums from all chunks within a stripe. It is currently stored on replica index 1 and the parity replicas.
5. BlockSize: The size of each block in a replica.
6. BlockGroupLen: The total combined length of all blocks across the data replicas within a BlockGroup.
7. PutBlock: At the end of every successful stripe write, a PutBlock command is sent to update the blockGroupLength to all the replica indexes and to update the stripe checksum to first replica and parity replicas.

## Guiding Principles
In addition to the [principles for replicated containers](https://github.com/apache/ozone/blob/a4bb94ee6390741f3b454f62dfd5a80a0d0b69d8/hadoop-hdds/docs/content/design/container-reconciliation.md#guiding-principles), the following guidelines will steer the design and implementation for EC container reconciliation:

1. **Reconciliation over garbage collection**:  The process should ensure that the data is recoverable and consistent across the replicas rather than cleaning the partially written or duplicate data.
...
- The size of a stripe might not need data chunks to be written to each member of the stripe. In this case even though a chunk is not written, the client is expected to send a `PutBlock` call to ALL the replicas after the data and parity chunks are written to a subset of the replicas participating in the stripe.
- Each Datanode can count the number of chunks in a block as long as the client was able to complete all PutBlocks. This is due to the point above that each row in the stripe will be completed via a PutBlock call.
- If the container for a replica index is missing, the background processing will reconstruct that replica index. Reconstruction of a replica index is atomic. The container for that replica index is reported to SCM only after all blocks have been repaired.

## Merkle Tree for EC Containers
To detect inconsistency in an EC layout, we cannot rely on chunk checksums as they are not equal across the replica index. This is the primary reason why reconciliation for EC needs its own design different that replication based containers.

Instead, we propose generating the container-level metadata checksum using the metadata across replica indexes, which is sufficient for identifying mismatches across replicas.

This proposal extends the existing container reconciliation framework for replicated containers to support EC containers by introducing metadata-based checksums at each level of the Merkle tree. Instead of relying on actual data content, chunk-level checksums are derived from chunk offsets and health status, and are aggregated to compute block-level and container-level metadata checksums. Thus, locally the chunks are validated for correctness using the chunk checksum recorded at the time of write but the offset information is used to validate the completeness of the block across the replicas.
...
**Note:** The idea here is to keep the scanner localized and agnostic of the underlying data layout, EC vs Replication. Scanner's responsibility is just to compute and report container checksums (as seen locally) to SCM. SCM will then determine which checksum (between data checksum and metadata checksum) is needed to detect inconsistencies and initiate reconciliation if required. SCM will delegate the actual reconciliation to the Datanodes themselves.

### Merkle Tree with metadata checksum
```protobuf
...
### Sample code to calculate metadata checksum for chunk, block and container
```java
public long chunkMetadataChecksum(long chunkOffset, boolean isHealthy) {
   ChecksumByteBuffer checksumImpl = CHECKSUM_BUFFER_SUPPLIER.get();
   checksumImpl.update(isHealthy ? 1 : 0);
   ByteBuffer buffer = ByteBuffer.allocate(Long.BYTES);
   buffer.putLong(chunkOffset);
   buffer.flip();
   checksumImpl.update(buffer);
   return checksumImpl.getValue();
}

public long blockMetadataChecksum(Collection<ContainerProtos.ChunkMerkleTree> chunkMerkleTrees) {
   ChecksumByteBuffer checksumImpl = CHECKSUM_BUFFER_SUPPLIER.get();
   ByteBuffer blockMetadataChecksumBuffer = ByteBuffer.allocate(Long.BYTES * chunkMerkleTrees.size());

   for (ContainerProtos.ChunkMerkleTree chunkTree : chunkMerkleTrees) {
      blockMetadataChecksumBuffer.putLong(chunkTree.getMetadataChecksum());
   }

   blockMetadataChecksumBuffer.flip();
   checksumImpl.update(blockMetadataChecksumBuffer);
   return checksumImpl.getValue();
}

public long containerMetadataChecksum(Collection<ContainerProtos.BlockMerkleTree> blockMerkleTrees) {
   ChecksumByteBuffer checksumImpl = CHECKSUM_BUFFER_SUPPLIER.get();
   ByteBuffer containerMetadataChecksumBuffer = ByteBuffer.allocate(Long.BYTES * blockMerkleTrees.size());

   for (ContainerProtos.BlockMerkleTree blockTree : blockMerkleTrees) {
      containerMetadataChecksumBuffer.putLong(blockTree.getMetadataChecksum());
   }
   containerMetadataChecksumBuffer.flip();
   checksumImpl.update(containerMetadataChecksumBuffer);
   return checksumImpl.getValue();
}
```
...
|                       | PutBlock 1       | PutBlock 1       | PutBlock 1       | PutBlock 1                  | PutBlock 1                  | PutBlock sent to all node |
| WriteChunk 2          | r1c2             | r2c2             | r3c2             | r4c2                        | r5c2                        | Entire Row written        |
|                       | PutBlock 2       | PutBlock 2       | PutBlock 2       | PutBlock 2                  | PutBlock 2                  | PutBlock sent to all node |
| WriteChunk 3          | r1c3             | r2c3             | r3c3             | r4c3                        | r5c3                        | Entire Row written        |
|                       | PutBlock 3       | PutBlock 3       | PutBlock 3       | PutBlock 3                  | PutBlock 3                  | PutBlock sent to all node |
...
|                       | PutBlock 3       | PutBlock 3       | PutBlock 3       | PutBlock 3                  | PutBlock 3                  | Partial Row written but PutBlock sent to all node |
...
Going forward, the replication configuration will be stored on the DataNode at the time of container creation to avoid the need of additional calls to SCM.

## Reconciliation for Erasure Coded containers
Reconciliation for EC containers can occur in two scenarios:
1. Recoverable stripe: A stripe is recoverable when the majority of replicas are healthy.
...
2. Unrecoverable stripe: A stripe is not recoverable when the majority of replicas are unhealthy.
   - If unrecoverable due to an insufficient number of healthy replicas, reconciliation updates the Block Data to indicate its unrecoverable status. This ensures:
...
1. **Container is missing chunks or has corrupted chunks**: 
   ...
   |                       | PutBlock 2       | PutBlock 2       | PutBlock 2       | PutBlock 2                  | PutBlock 2                  | Partial Row written but PutBlock sent to all node           |
   | **Block ID 3**        |
   | WriteChunk 1          | r1c1             |                  |                  | r4c1 (missing)              | r5c1                        | Partial row written (Has missing but recoverable chunk)     |
   |                       | PutBlock 1       | PutBlock 1       | PutBlock 1       | PutBlock 1                  | PutBlock 1                  | Partial Row written but PutBlock sent to all node           |

In all the above scenarios, stripe is recoverable. On a datanode, reconciliation process will indentify the missing or corrupted chunk/s and then fetch the data form healthy replicas/parities to generate the missing or corrupted chunk/s. It will also update the block level metadata checksum in the Merkle tree to reflect that stripe is recovered.

2. **Container has corrupted blocks**: 
    Corrupted block is nothing but an extended case of missing or corrupted chunks. In this case, reconciliation process will fetch block's data chunk by chunk, and reconstruct the block and then mark the stripe recovered.

### Unrecoverable stripe:
...
|                       | PutBlock 1                | PutBlock 1                | PutBlock 1                     | PutBlock 1           | PutBlock 1                  | PutBlock 1                  | PutBlock sent to all node                               |
| WriteChunk 2          | r1c2                      | r2c2                      | r2c2                           | r3c2                 | r4c2                        | r5c2                        | Entire Row written                                      |
|                       | PutBlock 2                | PutBlock 2                | PutBlock 2                     | PutBlock 2           | PutBlock 2                  | PutBlock 2                  | PutBlock sent to all node                               |
...
|                       | PutBlock 3                | PutBlock 3                | PutBlock 3                     | PutBlock 3           | PutBlock 3                  | PutBlock 3                  | Partial Row written but PutBlock sent to all node       |
...
| **Block ID**          |
| WriteChunk 1          | r1c1            | r2c1                       | r2c1                       | r3c1                                               | r4c1                       | r5c1                       | Entire Row written                                |
|                       | PutBlock 1      | PutBlock 1                 | PutBlock 1                 | PutBlock 1                                         | PutBlock 1                 | PutBlock 1                 | PutBlock is present on all nodes                  |
| WriteChunk 2          | r1c2            | r2c2                       | r2c2                       | r3c2                                               | r4c2                       | r5c2                       | Entire Row written                                |
|                       | PutBlock 2      | PutBlock 2                 | PutBlock 2                 | PutBlock 2                                         | PutBlock 2                 | PutBlock 2                 | PutBlock is present on all nodes                  |
...
| WriteChunk 2          | r1c2             | r2c2                 | r3c2             | r4c2                        | r5c2                        | Entire Row written                                |
| WriteChunk 3          | r1c3             | r2c3                 | r3c3             | r4c3                        | r5c3                        | Entire Row written                                |
|                       | PutBlock         |                      |                  |                             | PutBlock                    | PutBlock is missing on majority of the DataNodes. |
...
| WriteChunk 2          | r1c2             | r2c2                 | r3c2             | r4c2                        | r5c2                        | Entire Row written                                |
| WriteChunk 3          | r1c3             | r2c3                 | r3c3             | r4c3                        | r5c3                        | Entire Row written                                |
|                       | PutBlock         | PutBlock             | PutBlock         | PutBlock                    | PutBlock                    | PutBlock is missing on majority of the DataNodes. |
...
| WriteChunk 1          | r1c1             | r2c1             | r3c1             | r4c1                        | r5c1                        | Entire Row written          |
| WriteChunk 2          | r1c2             | r2c2             | r3c2             | r4c2                        | r5c2                        | Entire Row written          |
| WriteChunk 3          | r1c3             |                  |                  | MISSING                     | r5c3                        | The metadata checksum match |
|                       | PutBlock         |                  |                  |                             | PutBlock                    | Partial Row written         |
...
| WriteChunk 2          | r1c2             | r2c2             | r3c2             | r4c2                                    | r5c2                        | Entire Row written                                |
| WriteChunk 3          | r1c3             |                  |                  | MISSING                                 | r5c3                        | The metadata checksum match                       |
|                       | PutBlock         | PutBlock         | PutBlock         | PutBlock                                | PutBlock                    | Partial Row written but PutBlock sent to all node |
...
|                       | PutBlock         |                  |                  |                             | PutBlock                    | Partial PutBlock written |
| Metadata Checksum     | value2 (ok)      | value1           | value1           | value1                      | value2 (ok)                 | value2 (ok)              | 
