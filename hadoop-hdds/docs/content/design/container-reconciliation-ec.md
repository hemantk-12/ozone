...
7. PutBlock: At the end of a every successful stripe write, a PutBlock command is sent to update the blockGroupLength to all the replica indexes and to update the stripe checksum to first replica and parity replicas.
...
**Note**: A PutBlock is sent after each stripe, but it is shown after the last stripe in all the below tables for simplicity.
